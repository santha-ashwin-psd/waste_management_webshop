...
from webshop_waste.webshop_waste.doctype.override_doctype.item_group import invalidate_cache_for

class DataValidationError(frappe.ValidationError):
	pass

class webshop_wasteItem(Item):
	def on_update(self):
		super(webshop_wasteItem, self).on_update()
		invalidate_cache_for_item(self)
		super(webshop_wasteItem, self).on_update()

	def before_rename(self, old_name, new_name, merge=False):
		self.validate_duplicate_website_item_before_merge(old_name, new_name)
		return super(webshop_wasteItem, self).before_rename(old_name, new_name, merge)
	...
	def after_rename(self, old_name, new_name, merge):
		...
		super(webshop_wasteItem, self).after_rename(old_name, new_name, merge)
